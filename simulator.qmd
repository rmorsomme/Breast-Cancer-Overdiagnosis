...
d_obs_screen
d_obs_censor
count(d_obs_censor, censor_type) # (80-2-18)
count(d_process, indolent)
```

```{r}
id_clinical <- which(d_obs_censor$censor_type == "clinical")
id_censor   <- which(d_obs_censor$censor_type == "censored")
id_screen   <- which(d_obs_censor$censor_type == "screen")
```
